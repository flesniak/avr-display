...
void usiTwiSlaveInit(uint8_t ownAddress) {
  ...
  USICR =
       ...
       (1 << USICS1) | (0 << USICS0) |		// Shift Register Clock Source = external, positive edge 4-Bit Counter Source = external, both edges
       (0 << USICLK) | (0 << USITC);		// No toggle clock-port pin
...
ISR(USI_START_VECTOR) {
	...
	if (!(PIN_USI & (1 << PIN_USI_SDA))) {			// A Stop Condition did not occur
		...
		(1 << USICS1) | (0 << USICS0) |				// 4-Bit Counter Source = external, both edges; Clock Source = External, positive edge
		(0 << USICLK) | (0 << USITC);				// No toggle clock-port pin
...
//###### Address mode: check address and send ACK (and next USI_SLAVE_SEND_DATA) if OK, else reset USI
		case USI_SLAVE_CHECK_ADDRESS:
			if (USIDR == 0 || (USIDR & ~1) == slaveAddress) {   // If adress is either 0 or own address
				if (USIDR & 0x01) {
					overflowState = USI_SLAVE_SEND_DATA;		// Master Read Data Mode - Slave transmit
				} else {
					overflowState = USI_SLAVE_REQUEST_DATA;		// Master Write Data Mode - Slave receive
...
//###################################### Master Write Data Mode - Slave transmit
		...
		// From here we fall straight into USI_SLAVE_SEND_DATA if the master sent an ACK
		case USI_SLAVE_SEND_DATA:
			if (buffer_adr >= buffer_size)	// No buffer position given, set buffer address to 0
				buffer_adr=0;
...
//######################################## Master Read Data Mode - Slave receive
		...
		case USI_SLAVE_GET_DATA_AND_SEND_ACK:
			...
			if (buffer_adr >= buffer_size) {	// First access, read buffer position
